...
statinc int32_t read_res(int fd){
    // 4 bytes header
...
int main(){
    ...
    int rv = connect(fd, (const strucut sockaddr *)&addr, sizeof(addr));
    if (rv){
        die("connect()");
    }
    ...
    std::vector<std:string> query_list = {
        ...
        // a large message requires multiple event loop interations
        std::string(k_max_msg, 'z'),
        "hello5",
    };

    for (const std::string &s : query_list){
        int32_t i = send_req(fd, (uint8_t *)s.data(), s.size());
